function [pool] = update_pool(pop_X, chosen_X, chosen_y, index_SSM, pool, params)
...
switch rule_choose_sample
    ...
    case 'k_nearest'
        [pool] = keeping_lowest(chosen_X, chosen_y, pool, params);
%        [pool] = keeping_newest(chosen_X, chosen_y, pool, params);
%        [pool] = keeping_local_best(pop_X, chosen_X, chosen_y, index_SSM, pool, params);
end
...
%
function [pool] = keeping_local_best(pop_X, chosen_X, chosen_y, index_SSM, pool, params)
% KEEPING_LOCAL_BEST: Upadate the pool of solutions.
% This function updates the pool by keeping the best solutions regarding the
% sample that was used to buil he local metamodel.
% 
% Pool size N_pool and pop_X size N_pop
[N_pool, n] = size(pool.X);
[N_pop, ~] = size(pop_X);
...
for i = 1:N_chosen
    ...
    if d <= params.tol_ratio
        ...
        if N_pool < params.max_pool_size
            ...
            % Find the worst solution in the neighborhood of chosen_X(i,:)
            idx_ssm = index_SSM(i).index;
            [value, idx] = max(pool.y(idx_ssm));
