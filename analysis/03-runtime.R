...
## the runtime required by each approach.
##
## Contributors:
## - Mônica A. C. Valadão
## - André L. Maravilha
...
aggdata <- aggdata %>%
  ...
  dplyr::select(PROB, NVAR, REP, RULE, ITER, NEVAL, TOTAL.TIME.S) %>%
  dplyr::group_by(PROB, NVAR, REP, RULE) %>%
  dplyr::mutate(DELTA.TIME.S = TOTAL.TIME.S / ITER) %>%
...
output.directory <- file.path(".", "figures", "03_runtime")
...
# =================================================================================================
# Boxplot
# Mean runtime per iteration (in seconds) vs. strategy

# Prepare data
fig.data <- aggdata %>%
  dplyr::mutate(PROB.NVAR = as.factor(sprintf("%s (%02d)", PROB, NVAR))) %>%
  dplyr::group_by(PROB.NVAR, RULE) %>%
  dplyr::summarise(DELTA.TIME.S = mean(DELTA.TIME.S)) %>%
  dplyr::group_by(RULE) %>%
  dplyr::mutate(IS.OUTLIER = is_outlier(DELTA.TIME.S)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(PROB.NVAR, RULE)

# Create figure
fig <- ggplot2::ggplot(fig.data, 
                       aes(x = RULE, y = DELTA.TIME.S, label = PROB.NVAR, fill = RULE)) +
  ...
  ggplot2::ylab("Mean runtime per iteration (in seconds)") +
  ggplot2::theme_bw() +
  ggplot2::theme(legend.position = "none",
                 axis.title = element_text(size = 16),
                 axis.text = element_text(size = 12, margin = margin(t = 6, b = 6))) #+
  #facet_zoom2(ylim = c(0, 35), zoom.size = 1, show.area = TRUE, shrink = TRUE, split = TRUE)
  #facet_zoom2(ylim = c(0, 3), zoom.size = 1, show.area = TRUE, shrink = TRUE, split = TRUE)

# Save figure as PDF
ggplot2::ggsave(file.path(output.directory, "runtime-boxplot-overall.pdf"),
                plot = fig, width = 10, height = 5)

# Save figure as PNG
ggplot2::ggsave(file.path(output.directory, "runtime-boxplot-overall.png"), 
                plot = fig, width = 10, height = 5)

# Save figure as eps
ggplot2::ggsave(file.path(output.directory, "runtime-boxplot-overall.eps"), 
                plot = fig, width = 10, height = 5)
# =================================================================================================
# Boxplot
# Mean runtime per iteration (in seconds) vs. strategy
...
# Create figure
fig <- ggplot2::ggplot(fig.data, aes(x = DELTA.TIME.S, y = RULE, fill = RULE)) +
  ggplot2::geom_boxplot() +
...
# Save figure as PDF
ggplot2::ggsave(file.path(output.directory, "runtime-boxplot-by_function_and_size.pdf"), 
                plot = fig, width = 210, height = 290, units = "mm")

# Save figure as PNG
ggplot2::ggsave(file.path(output.directory, "runtime-boxplot-by_function_and_size.png"), 
                plot = fig, width = 210, height = 290, units = "mm")

# Save figure as eps
ggplot2::ggsave(file.path(output.directory, "runtime-boxplot-by_function_and_size.eps"), 
                plot = fig, width = 210, height = 290, units = "mm")
# =================================================================================================
...
for (i1 in 1:num.levels) {
  for (i2 in 1:num.levels) {
    if (i1 < i2) {
      ...
      paired.diff <- mc.data$DELTA.TIME.S[mc.data$RULE == x1] - mc.data$DELTA.TIME.S[mc.data$RULE == x2]
...
fig <- ggplot2::ggplot(mc.ci, 
  ...
  ggplot2::coord_flip() + 
  xlab("Comparison") +
  ylab("Mean difference in runtime per iteration (in seconds)") +
  ggplot2::theme_bw() +
...
# Save figure as PDF
ggplot2::ggsave(file.path(output.directory, "runtime-ci.pdf"), 
                plot = fig, width = 10, height = 7)

# Save figure as PNG
ggplot2::ggsave(file.path(output.directory, "runtime-ci.png"), 
                plot = fig, width = 10, height = 7)

# Save figure as eps
ggplot2::ggsave(file.path(output.directory, "runtime-ci.eps"), 
                plot = fig, width = 10, height = 7)